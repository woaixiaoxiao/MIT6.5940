...
#include "../matmul.h"
#include "common.h"
signed char Getpost2(uint8_t a) { return (a >> 4) - 8; }
signed char Getpre2(uint8_t a) { return (a & 0x0F) - 8; }
        ...
        for (int col = mat_args->start; col < mat_args->end; col += 4) {
            ...
            for (int ch = 0; ch < k;) {
                ...
                for (int qj = 0; qj < 32; qj++) {
                    // TODO: decode a packed byte into two int8 in the range of (-8, 7)
                    intermediate_sum0 += Getpre2(w0_int4[qj]) * a_int8[qj];
                    intermediate_sum0_2nd += Getpost2(w0_int4[qj]) * a_int8[qj + 32];
                    intermediate_sum1 += Getpre2(w1_int4[qj]) * a_int8[qj];
                    intermediate_sum1_2nd += Getpost2(w1_int4[qj]) * a_int8[qj + 32];
                    intermediate_sum2 += Getpre2(w2_int4[qj]) * a_int8[qj];
                    intermediate_sum2_2nd += Getpost2(w2_int4[qj]) * a_int8[qj + 32];
                    intermediate_sum3 += Getpre2(w3_int4[qj]) * a_int8[qj];
                    intermediate_sum3_2nd += Getpost2(w3_int4[qj]) * a_int8[qj + 32];
                    // TODO: int8 multiply and accumulate operation
                }
                // dequantize the sum into floating point
                acc0 += (float)intermediate_sum0 * s_a * s_w0;
