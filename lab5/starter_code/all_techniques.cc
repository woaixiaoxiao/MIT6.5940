    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i lowbit_w_next = _mm256_and_si256(raw_w_next, lowMask);
                // 分别取出高位
                __m256i highbit_w = _mm256_and_si256(_mm256_srli_epi16(raw_w, 4), lowMask);
                __m256i highbit_w_next = _mm256_and_si256(_mm256_srli_epi16(raw_w_next, 4), lowMask);
                ...
                w_0_next = _mm256_sub_epi8(lowbit_w_next, zero_point);
                w_128_next = _mm256_sub_epi8(highbit_w_next, zero_point);
...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    for (int i = 0; i < num_thread; i++) {
        threads_args[i].start_j = n * i / num_thread;
        threads_args[i].end_j = n * (i + 1) / num_thread;
